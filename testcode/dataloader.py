...
class Loader():
    def __init__(self,config):
        ...
        self.__variable__(config)

    def __variable__(self,config):
        self.config = config
        self.spath = '../data/%s/s_final.csv'%(config.dataset)
        self.tpath = '../data/%s/t_final.csv'%(config.dataset)
    ...
    def build_dataset_cross_domain_step1(self):
        self.n_test_users = int(self.n_users * 0.2)
        self.n_train_users = self.n_users - self.n_test_users
        self.users = self.sdata['u'].value_counts().index._values
        self.test_users = random.sample(list(self.users), self.n_test_users)
        remain_users = list(set(self.users) - set(self.test_users))
        self.test_data = pd.DataFrame()
        self.train_data = pd.DataFrame()
        for u in self.test_users:
            self.test_data = self.test_data.append(self.tdata[self.tdata['u'] == u])
            self.train_data = self.train_data.append(self.sdata[self.sdata['u'] == u])
        for u in remain_users:
            self.train_data = self.train_data.append(self.sdata[self.sdata['u'] == u])
            self.train_data = self.train_data.append(self.tdata[self.tdata['u'] == u])
        ...
        self.train_data.to_csv('../data/amazon/train.csv',index=False,header=False)
        self.test_data.to_csv('../data/amazon/test.csv',index=False,header=False)

    def build_dataset_cross_domain_step2(self):
        self.train_data = pd.read_csv('../data/amazon/train.csv',names=['u','i','r','t'])
        self.test_data = pd.read_csv('../data/amazon/test.csv',names=['u','i','r','t'])
        ...
        self.train_data.to_csv('../data/amazon/test/train_cd.csv', index=False, header=False)
        self.test_data.to_csv('../data/amazon/test/test_cd.csv', index=False, header=False)
    ...
    def generate_dataset(self):
        ...
        if not os.path.exists('../data/amazon/test/train_cd.csv'):
            if not os.path.exists('../data/amazon/train.csv'):
                print('bulid dataset......')
                start = time()
                self.build_dataset_cross_domain_step1()
                print('bulid over! cost {:4f}s!'.format(time()-start))
            print('bulid cd dataset...')
            self.build_dataset_cross_domain_step2()
            print('bulid cd dataset over!')
        self.train_data = pd.read_csv('../data/amazon/test/train_cd.csv', ',', names=['u', 'i', 'r', 't'])
        self.test_data = pd.read_csv('../data/amazon/test/test_cd.csv', ',', names=['u', 'i', 'r', 't'])
        ...
        '''
        self.train_sdata['mode'] = 's'
        self.train_tdata['mode'] = 't'
        self.train_data_mode = self.train_sdata.append(self.train_tdata)'''
    ...
    def load_data(self,type):
        self.sdata = pd.read_csv(self.spath,names=['u','i','r','t'])
        self.tdata = pd.read_csv(self.tpath,names=['u','i','r','t'])
        ...
        #y_list = [[] for i in range(self.n_users)]
        #for i in range(self.tdata.shape[0]):
        #    y_list[self.tdata['u'].iloc[i]].append(self.tdata['i'].iloc[i])
        #self.y_list = np.array(y_list)
        self.generate_dataset()


class PairwiseDataset(torch.utils.data.Dataset):
    def __init__(self,loader,type,num_ng,is_training = None):
        super(PairwiseDataset,self).__init__()
        if type == 's':
            self.data = loader.sdata
            self.n_items = loader.n_sitems
            self.R = loader.sy
        else:
            self.data = loader.tdata
            self.n_items = loader.n_titems
            self.R = loader.ty

        self.is_training = is_training
        self.num_ng = num_ng
        self.users = self.data[0]
        self.items = self.data[1]
        self.data_labels_ps = [1 for _ in range(len(self.users))]
    ...
    def ng_sample(self):
        ...
        for (u,i) in zip(self.users,self.items):
            ...
            while (u, j) in self.R:
                j = np.random.randint(self.n_items)
            items_ng.append(j)
        self.data_ = (self.users,self.items, items_ng)
...
class PointwiseDataset(torch.utils.data.Dataset):
    def __init__(self,loader,data,num_ng,is_training = None):
        ...
        self.data = data
        self.n_items = loader.n_sitems + loader.n_titems
        self.y = loader.y
