...
def training(adata, tdata):
    df_adata,df_tdata = pd.DataFrame(), pd.DataFrame()
    df_adata["user"], df_adata["item"], df_adata["label"] = adata.final_data[0], adata.final_data[1], adata.final_data[2]
    df_tdata["user"], df_tdata["item"], df_tdata["label"] = tdata.final_data[0], tdata.final_data[1], tdata.final_data[2]

    train_frames = []
    for user, sf_data_1 in df_adata.groupby("user"):
        df = pd.DataFrame()
        sf_data_2 = df_tdata.loc[df_tdata["user"] == user]
        l = min(len(sf_data_1), len(sf_data_2))
        df["user"] = l * [user]
        sample_data_1 = sf_data_1.sample(n=l)
        sample_data_2 = sf_data_2.sample(n=l)
        df["aitem"] = sample_data_1["item"].values
        df["titem"] = sample_data_2["item"].values
        df["alabel"] = sample_data_1["label"].values
        df["tlabel"] = sample_data_2["label"].values
...
class CoNet(nn.Module):
    def __init__(self, adata, tdata):
        ...
        self.V_adata = nn.Embedding(adata.n_items, self.edim)
        self.V_tdata = nn.Embedding(tdata.n_items, self.edim)
        self.f = nn.Sigmoid()
...
model = CoNet(a_train_data, t_train_data)
model.to(device)
optimizer = torch.optim.Adam(model.parameters(), lr=0.001)
criterion = nn.BCELoss()

def pretrain(train_data, mode='t'):
    print('='*20,'pretrain-{}'.format(mode),'='*20)
...
for epoch in range(EPOCHS):
    ...
    data, labels = training(a_train_data,t_train_data)
    ...
    if epoch and epoch % 5 == 0:
        ...
        for testloader_index in range(4):
            ...
            for user, item in testloader[testloader_index]:
                ...
                rating = model.t_predict(user, item)
